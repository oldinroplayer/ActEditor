...
namespace ActEditor.Core.WPF.EditorControls.ActSelectorComponents {
	public static class ActSelectorHelper {
		...
		public static void InitSelectorComboBox(IFrameRendererEditor editor, ComboBox actionBox, ComboBox animationBox) {
			ActSelectorData data = new ActSelectorData();
			data.Editor = editor;
		...
		private static void _editor_ActLoaded(ActSelectorData data) {
			...
			data.Act.Commands.CommandIndexChanged += (s, c) => _actionCommands_CommandIndexChanged(s, c, data);

			int actions = data.Act.NumberOfActions;

			data.AnimationComboBox.ItemsSource = ActHelper.GetAnimations(data.Act);
		...
		private static void _updateActionSelection(ActSelectorData data) {
			try {
				...
				var animations = ActHelper.GetAnimations(data.Editor.Act);
